...
| DeskView | `0x06,0x10` | 効果なし。分割表示にならない。未解明 |
| ジンバルリセット | Sel 14 に `0x01` | **効果なし**。送信前後でフレームが同一、姿勢が変わらない |
...
**四隅座標は出力ではなく入力である。** カメラを物理的に動かしてから
古い座標を書き込むと、ボードを再検出せずに古い領域をそのまま切り出す。
自前で合成した座標も受理され、指定どおりの領域が切り出される。
ホワイトボードに限らず任意の四辺形を正対補正できる。

制約として、**縦長の領域は拒否される**（`byte[1]` が `0x01` に戻り座標がゼロクリアされる）。
横長の領域なら受理される。境界値は未調査。

### 実装上の注意

- **追跡 ON 中は V4L2 の `pan_absolute` / `tilt_absolute` が実位置を反映しない。**
  ジンバルが自律的に大きく動いていても値は変化しないため、
  PTZ による位置指定と追跡は併用できない。追跡を使うなら PTZ 側は無効にする
